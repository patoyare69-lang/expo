...
public final class JavaScriptValue: JavaScriptType, Equatable, Escapable, Error {
  ...
  /**
   Returns the value as a weak object, or asserts if not a weak object.
   */
  public func getWeakObject() -> JavaScriptWeakObject {
//    guard let runtime else {
      FatalError.runtimeLost()
//    }
//    assert(isObject(), "Value is not a weak object")
//    return JavaScriptWeakObject(runtime, pointee.getObject(runtime.pointee))
  }
  ...
  @JavaScriptActor
  public func asWeakObject() throws(TypeError) -> JavaScriptWeakObject {
    guard isObject() else {
      throw TypeError(type: JavaScriptWeakObject.self)
    }
    return getWeakObject()
  }
